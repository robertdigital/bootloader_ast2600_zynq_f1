...
void ast_scu_multi_func_romcs(u32 n)
{
...
//		printf ("- Warning: %d protected sectors will not be erased!\n", prot);
		nc_printf ("- Warning: protected sectors will not be erased!\n");
		print_reg(prot);
	} else {
//		putc ('\n');
		nc_printf("\n");
	...
	/*ashok changed this to erase exact no of sectors*/
	for (sect = s_first; sect < s_last; sect++) {
//	for (sect = s_first; sect <= s_last; sect++) {
//		print_reg(sect);
...
//	nc_printf("SPI regbase ");
//	print_reg(info->reg_base);
	...
	*(ulong *) (info->reg_base + CtrlOffset) = ulCtrlData;
	...
	udelay(200);
	*(volatile uchar *) (vbase) = (uchar) (0x9F);
	udelay(10);
...
//			printf("Unsupported SPI Flash!! 0x%08lx\n", info->flash_id);
			printf("Unsupported SPI Flash!!");
			print_reg(info->flash_id);
			erase_region_size  = 0x10000;
			info->readcmd = 0x0b;
			info->dualport  = 0;
			info->dummybyte = 1;
	...
	if (info->address32) {
#ifndef AST_SOC_G5
		reg = *((volatile ulong*) HW_TRAPPINGS);	/* set H/W Trappings */
		reg |= 0x10;
		*((volatile ulong*) HW_TRAPPINGS) = reg;
#endif
...
void memmove_dma(void * dest,const void *src, u32 count)
{
